...
/* primitives.jsx - UI kit: Icon, Button, Card, Badge, Spinner, PageLoader, Skeleton, EmptyState, SectionHeader, ErrorState */
/* Each script has its own scope - we assign to window at the end */

// ---- ICON ---- (using simple SVG path icons inline)
...
// ---- CARD ----
const Card = ({ children, className = '', style = {}, onClick, pad = true }) => (
  <div
...
// ---- BADGE ----
const Badge = ({ children, variant = 'blue', icon }) => (
...
// ---- SKELETON ----
const Skeleton = ({ width = '100%', height = 16, radius = 'var(--radius-md)', style = {} }) => (
  <div
    style={{
      width,
      height,
...
// ---- EMPTY STATE ----
const EmptyState = ({ icon = 'list', title = 'Nothing here', sub = '', action }) => (
...
// ---- KPI CARD ----
const KPICard = ({ label, value, icon, footer, trend }) => (
...
// ---- MODAL ----
const Modal = ({ open, onClose, title, children, footer, wide = false, danger = false }) => {
  if (!open) return null;
  ...
  return (
    <div className="hid-modal-scrim" onClick={handleScrim}>
      <div className={['hid-modal', danger ? 'hid-breakglass-modal' : ''].filter(Boolean).join(' ')}
        ...
        {title && (
          <div className={['hid-modal-head', danger ? 'hid-breakglass-head' : ''].filter(Boolean).join(' ')}>
            ...
            <button className="hid-btn hid-btn-ghost hid-btn-sm hid-btn-icon" onClick={onClose} style={{ flexShrink: 0 }}>
              <Icon name="x" size={16} />
            </button>
          </div>
        )}
...
// ---- INPUT (controlled) ----
const Input = ({ value, onChange, placeholder, type = 'text', error, disabled, icon, className = '' }) => (
...
const ToastProvider = ({ children }) => {
  ...
  return (
    <>
      {children}
      <div style={{ position: 'fixed', top: 'var(--space-600)', right: 'var(--space-600)', zIndex: 'var(--z-toast)', display: 'flex', flexDirection: 'column', gap: 'var(--space-300)' }}>
        {toasts.map(t => (
          <div key={t.id} style={{
...
.ps-8 { padding-left: 36px !important; }
`;
document.head.appendChild(skeletonStyle);



export { Icon, Button, Card, Badge, Spinner, PageLoader, Skeleton, EmptyState, SectionHeader, ErrorState, PageHead, KPICard, Status, Modal, TabBar, Field, Input, Select, Textarea, ToastProvider, showToast };
