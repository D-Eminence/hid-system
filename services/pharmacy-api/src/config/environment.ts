...
const environmentSchema = z.object({
  ...
  IDENTITY_PHARMACY_WORKLOAD_TOKEN_FILE: optionalString,
  ...
  PHARMACY_WORKLOAD_ISSUER_URL: optionalUrl,
  PHARMACY_WORKLOAD_AUDIENCE: optionalString,
  PHARMACY_WORKLOAD_JWKS_URL: optionalUrl,
  PHARMACY_EHR_CALLER_SUBJECT: optionalString,
  PHARMACY_OCR_CALLER_SUBJECT: optionalString,
}).superRefine((environment, context) => {
  if (environment.PHARMACY_SERVICE_IDENTITY_MODE === 'local-secret'
      && !environment.PHARMACY_INTERNAL_SERVICE_TOKEN) {
    context.addIssue({ code: 'custom', path: ['PHARMACY_INTERNAL_SERVICE_TOKEN'],
      message: 'Local service identity requires an ephemeral secret' });
  }
  if (environment.PHARMACY_SERVICE_IDENTITY_MODE === 'jwt') {
    for (const key of ['PHARMACY_WORKLOAD_ISSUER_URL', 'PHARMACY_WORKLOAD_AUDIENCE',
      'PHARMACY_WORKLOAD_JWKS_URL', 'PHARMACY_EHR_CALLER_SUBJECT', 'PHARMACY_OCR_CALLER_SUBJECT'] as const) {
      if (!environment[key]) context.addIssue({ code: 'custom', path: [key],
        message: `${key} is required for JWT workload identity` });
    }
  }
  if (environment.IDENTITY_SERVICE_IDENTITY_MODE === 'jwt'
      && !environment.IDENTITY_PHARMACY_WORKLOAD_TOKEN_FILE) {
    context.addIssue({ code: 'custom', path: ['IDENTITY_PHARMACY_WORKLOAD_TOKEN_FILE'],
      message: 'IDENTITY_PHARMACY_WORKLOAD_TOKEN_FILE is required for JWT Identity calls' });
  }
  if (environment.NODE_ENV === 'production') {
    ...
    if (environment.PHARMACY_SERVICE_IDENTITY_MODE !== 'jwt') context.addIssue({ code: 'custom',
      path: ['PHARMACY_SERVICE_IDENTITY_MODE'], message: 'Production Pharmacy API requires JWT workload identity' });
    if (environment.IDENTITY_SERVICE_IDENTITY_MODE !== 'jwt') context.addIssue({ code: 'custom',
      path: ['IDENTITY_SERVICE_IDENTITY_MODE'], message: 'Production Identity calls require JWT workload identity' });
    if (environment.PHARMACY_INTERNAL_SERVICE_TOKEN) context.addIssue({ code: 'custom',
      path: ['PHARMACY_INTERNAL_SERVICE_TOKEN'], message: 'Local shared-token authentication is forbidden in production' });
    if (environment.IDENTITY_PHARMACY_INTERNAL_SERVICE_TOKEN) context.addIssue({ code: 'custom',
      path: ['IDENTITY_PHARMACY_INTERNAL_SERVICE_TOKEN'], message: 'Local Identity service tokens are forbidden in production' });
    ...
    for (const [key, value] of [['IDENTITY_API_URL', environment.IDENTITY_API_URL],
      ['PHARMACY_WORKLOAD_ISSUER_URL', environment.PHARMACY_WORKLOAD_ISSUER_URL],
      ['PHARMACY_WORKLOAD_JWKS_URL', environment.PHARMACY_WORKLOAD_JWKS_URL]] as const) {
      if (value && new URL(value).protocol !== 'https:') context.addIssue({ code: 'custom', path: [key],
...
export function getEnvironment(): Environment {
  ...
  const result = environmentSchema.safeParse({ ...process.env,
    DATABASE_URL: process.env.PHARMACY_DATABASE_URL ?? process.env.DATABASE_URL });
  if (!result.success) throw new Error(`Invalid Pharmacy configuration: ${result.error.issues
