...
export class LabApiService {
  ...
  constructor(workloadIdentity:ServiceWorkloadIdentityService){const environment=getEnvironment();this.client=new LabApiClient({baseUrl:environment.LAB_API_URL,timeoutMs:10_000,internalServiceToken:environment.LAB_INTERNAL_SERVICE_TOKEN,
    ...(environment.LAB_SERVICE_IDENTITY_MODE==='jwt'?{serviceAuthorizationProvider:(caller)=>workloadIdentity.authorization('lab',caller)}:{})});}
  createOcrImport(context:DataAccessContext,input:unknown,key:string) {return this.translate(this.client.createOcrImport(input,this.context(context),key));}
