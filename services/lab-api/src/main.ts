...
async function bootstrap(): Promise<void> {
  ...
  app.useLogger(new Logger('HidLabApi'));
  const expressApplication = app.getHttpAdapter().getInstance() as Express;
  expressApplication.set(
    'trust proxy',
    environment.TRUST_PROXY_CIDRS
      ? environment.TRUST_PROXY_CIDRS.split(',').map((cidr) => cidr.trim())
      : false,
  );
  ...
  app.useGlobalPipes(new ValidationPipe({
    ...
    exceptionFactory: (errors) => new DomainProblem(
      400,
      'VALIDATION_FAILED',
