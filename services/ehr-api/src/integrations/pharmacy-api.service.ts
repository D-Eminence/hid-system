  ...
  constructor(workloadIdentity: ServiceWorkloadIdentityService) {
    ...
    this.client = new PharmacyApiClient({
      ...
      ...(environment.PHARMACY_SERVICE_IDENTITY_MODE === 'jwt'
        ? { serviceAuthorizationProvider: () => workloadIdentity.authorization('pharmacy') }
        : {}),
    });
  }

  acceptEhrPrescription(context: DataAccessContext, input: unknown, key: string) {
    return this.translate(this.client.acceptEhrPrescription(input, this.context(context), key));
  }
