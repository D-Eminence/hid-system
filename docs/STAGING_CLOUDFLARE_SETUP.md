...
diagnostic at **2026-09-22 03:02 UTC** completed all twenty inventory requests
with HTTP 200. The exact active zone/account is verified. All nine exact staging
DNS queries and all eight filtered Custom Domain queries returned zero records;
the zone's Worker Routes list is empty. One Turnstile widget exists, but none
matches the prepared seven-host staging domain set. The owner updated the
operator token's permissions; the checker made no resource changes. All deployments remain held for the pending
...
without HTTP details; two later runs returned HTTP 403 on the non-DNS reads.
The owner then changed the token permissions, and all API reads succeeded.
The new diagnostic supersedes those authentication/inventory failures.
The older agent readback used a failed Wrangler credential, which is a
...
metadata, rejects contradictory or partial totals, and never infers deployment
readiness from empty inventory.
...
For the staging widget, use name **HID staging**, mode **Managed**, and these exact
hostnames:
...
The verified operator inventory uses Zone DNS Read and Workers Routes Read plus
Account Workers Scripts Read and Turnstile Sites Read. An additional Zone Read
grant is not required by the zone request when DNS Read is present. API-based
widget creation would additionally need Turnstile Sites Write; inventory success
does not establish that write permission. Use the specific Turnstile permission
rather than general Account Settings Write. DNS Edit remains a later staging
setup permission; DNS/publication changes stay held for the quota gate.
[Cloudflare token permissions](https://developers.cloudflare.com/fundamentals/api/reference/permissions/).

Keep using a user-owned token for combined inventory: Cloudflare's compatibility
matrix lists Turnstile as unsupported by account-owned tokens. The protected
publisher is separate and does not require Turnstile access.
...
python3 scripts/check-staging-cloudflare-access.py --prompt-token \
  --output release/local/cloudflare-access-after-widget.json
```
